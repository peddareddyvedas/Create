...
public class OrganizationUpdateBankDetails extends AppCompatActivity implements View.OnClickListener {
    ...
    String  rs_accountname,rs_accountnumber,rs_bankname,rs_bankbranch,rs_ifsc;
    ApiDao apiDao;
    String addressId,from;
    Bundle bundle;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.organization_updatebank_details);
...
//        setTitle("Edit Bank Details");
        ...
        titleTv.setText("Edit Bank Details");
    ...
    private void set_previousdetails() {
        ...
        Call<Listmodel> set_previousdetails = apiDao.get_profile_bankdetails("Bearer "+AccountUtils.getAccessToken(this),addressId);
        set_previousdetails.enqueue(new Callback<Listmodel>() {
            @Override
            public void onResponse(Call<Listmodel> call, Response<Listmodel> response) {
                ...
                if (statuscode == HttpsURLConnection.HTTP_CREATED||statuscode==HttpsURLConnection.HTTP_OK||statuscode == HttpsURLConnection.HTTP_ACCEPTED){
                    for (Listmodel listmodel : list){
                        rs_accountname = listmodel.getName_on_account();
                        rs_accountnumber = listmodel.getAccount_number();
                        rs_bankname = listmodel.getBank_name();
                        rs_bankbranch = listmodel.getBank_branch();
                        rs_ifsc = listmodel.getIfsc_code();
                        et_accountname.setText(rs_accountname);
                        et_accountnumber.setText(rs_accountnumber);
                        et_bankname.setText(rs_bankname);
                        et_bankbranch.setText(rs_bankbranch);
                        et_ifsc.setText(rs_ifsc);
                        dialog.dismiss();
                    }
                }else {
                    dialog.dismiss();
                    ToastMessage.onToast(OrganizationUpdateBankDetails.this,"Serever error",ToastMessage.ERROR);
                }
            }
            ...
            public void onFailure(Call<Listmodel> call, Throwable t) {
                dialog.dismiss();
                ToastMessage.onToast(OrganizationUpdateBankDetails.this,"We have some issues",ToastMessage.ERROR);
            }
        });
    }
    ...
    public void init_validation(){
        ...
        timer.schedule(new TimerTask() {
            ...
            public void run() {
                ...
                OrganizationUpdateBankDetails.this.runOnUiThread(new Runnable() {
                    @Override
                    public void run() {
                        validation();
    ...
    public void validation(){
        ...
        if (st_accountname.isEmpty()){
            ...
            tv_accountname.setText("Please enter account Name");
        }
            ...
            tv_branch.setText("Please enter Branch");
        }
    ...
    private void editdetails() {
        ...
        dialog.show();
        apiDao = ApiClient.getClient(AccountUtils.getAccessToken(this)).create(ApiDao.class);
        Call<Listmodel> editdetails = apiDao.get_editbankdetails("Bearer "+AccountUtils.getAccessToken(this),addressId,
                st_accountnumber,st_accountname,st_bankname,st_bankbranch,st_ifsc);
        editdetails.enqueue(new Callback<Listmodel>() {
            @Override
            public void onResponse(Call<Listmodel> call, Response<Listmodel> response) {
                int statuscode = response.code();
                List<Listmodel> list = Collections.singletonList(response.body());
                if (statuscode == HttpsURLConnection.HTTP_CREATED||statuscode == HttpsURLConnection.HTTP_OK||statuscode==HttpsURLConnection.HTTP_ACCEPTED){
                    for (Listmodel listmodel : list){
                        dialog.dismiss();
                        ToastMessage.onToast(OrganizationUpdateBankDetails.this,"Successfully Updated..",ToastMessage.SUCCESS);
                        onsuccess();
                    }
                }
                else {
                    try {
                        ...
                        ToastMessage.onToast(OrganizationUpdateBankDetails.this, st, ToastMessage.ERROR);
    ...
    private void onsuccess() {
        Intent intent = new Intent(this, OrganizationBankDetails.class);
